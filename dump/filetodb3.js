...
function generateCreateTableQuery(data, tableName) {
  const columnTypes = detectColumnTypes(data);
  let columns = Object.keys(columnTypes).map(key => {
    return `\`${key}\` ${columnTypes[key]}`;
  }).join(', ');

  return `CREATE TABLE IF NOT EXISTS ${tableName} (column_id INT AUTO_INCREMENT PRIMARY KEY, ${columns});`;
}
...
app.post('/upload', async (req, res) => {
  ...
  try {
    ...
    const tableName = fileName.split('.').shift(); // Extract table name from the file name
    const createTableQuery = generateCreateTableQuery(data, tableName);
